...
describe("(IntegrationTest): /api/user POST", () => {
  ...
  it("should not create user if duplicate key violation on username", async () => {
    ...
    const duplicate_response = await request(app).post("/api/user").send({
      username: given_username,
      name: given_name,
      email: given_email,
      password: given_password,
...
/*
describe("(IntegrationTest): /api/user GET", () => {
...
describe("(IntegrationTest): /api/user:id GET", () => {
  it("should retrieve user given valid id", async () => {
    var given_username = "testUserName";
    var given_name = "Tests Alot";
    var given_email = "TESTING@email.com";
    var given_password = "test123";

    const user_response = await request(app).post("/api/user").send({
      username: given_username,
      name: given_name,
      email: given_email,
      password: given_password,
    ...
    testUserId = user_response.body.id;
